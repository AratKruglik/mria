...
@section('title')
  @if(isset($item)){{ $item->name }} | @endif Товары
@stop
  ...
  <div class="col-lg-12">
    <section class="panel">
      <header class="panel-heading">
        @if(isset($item)) {{ $item->name }} - Изменить товар @else Добавить товар @endif
      </header>
      <div class="panel-body">
        ...
        @else 
          {{ Form::open(['action' => 'DashboardController@goodAdd', 'method' => 'post', 'enctype' => 'multipart/form-data', 'class' => 'form-horizontal tasi-form']) }} 
        @endif
        
        @include('admin.forms.goods')
        {{ Form::close() }}
      </div>
    </section>
  </div>
