...
@section('title')
  @if(isset($item)){{ $item->name }} @else Додати @endif | Сторінка сайту
@stop
  ...
  <div class="col-lg-12">
    <section class="panel">
      <header class="panel-heading">
        @if(isset($item)) {{ $item->name }} - Змінити сторінку @else Додати сторінку @endif
      </header>
      <div class="panel-body">
        ...
        @else 
          {{ Form::open(['action' => 'DashboardController@pageAdd', 'method' => 'post', 'class' => 'form-horizontal tasi-form']) }} 
        @endif
        
        @include('admin.forms.pages')
        {{ Form::close() }}
      </div>
    </section>
  </div>
