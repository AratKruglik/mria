...
@section('title')
  @if(isset($item)){{ $item->name }} @else Додати @endif | Агенти
@stop
  ...
  <div class="col-lg-12">
    <section class="panel">
      <header class="panel-heading">
        @if(isset($item)) {{ $item->name }} - Редагувати агента @else Додати агента @endif
      </header>
      <div class="panel-body">
        ...
        @else 
          {{ Form::open(['action' => 'DashboardController@agentAdd', 'method' => 'post', 'class' => 'form-horizontal tasi-form']) }} 
        @endif
        
        @include('admin.forms.agents')
        {{ Form::close() }}
      </div>
    </section>
  </div>
