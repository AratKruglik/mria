...
@section('title')
  @if(isset($item)){{ $item->name }} @else Додати @endif | Об'єкт нерухомості
@stop
  ...
  <div class="col-lg-12">
    <section class="panel">
      <header class="panel-heading">
        @if(isset($item)) {{ $item->name }} - Змінити об'єкт @else Додати об'єкт @endif
      </header>
      <div class="panel-body">
        ...
        @else 
          {{ Form::open(['action' => 'DashboardController@objectAdd', 'method' => 'post', 'enctype' => 'multipart/form-data', 'class' => 'form-horizontal tasi-form']) }} 
        @endif
        
        @include('admin.forms.objects')
        {{ Form::close() }}
      </div>
    </section>
  </div>
